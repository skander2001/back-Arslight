...
class MongoClient(common.BaseObject):
    ...
    def _cache_credentials(self, source, credentials, connect=False):
        ...
        if source in all_credentials:
            ...
            raise OperationFailure(
                "Another user is already authenticated "
    ...
    def set_cursor_manager(self, manager_class):
        ...
        """
        warnings.warn(
            "set_cursor_manager is Deprecated", DeprecationWarning, stacklevel=2
        )
    ...
    def _get_socket(self, server, session):
        ...
        with _MongoClientErrorHandler(self, server, session) as err_handler:
            ...
            with server.get_socket(
                self.__all_credentials, handler=err_handler
            ) as sock_info:
    ...
    def _retry_with_session(self, retryable, func, session, bulk):
        ...
        """
        retryable = (
            retryable and self.retry_writes and session and not session.in_transaction
        )
    ...
    def _retryable_read(self, func, read_pref, session, address=None, retryable=True):
        ...
        """
        retryable = (
            retryable and self.retry_reads and not (session and session.in_transaction)
        )
        last_error = None
        retrying = False
