...
class Collection(common.BaseObject):
    ...
    def initialize_ordered_bulk_op(self, bypass_document_validation=False):
        ...
        """
        warnings.warn(
            "initialize_ordered_bulk_op is deprecated", DeprecationWarning, stacklevel=2
        )
    ...
    def _legacy_write(self, sock_info, name, cmd, op_id, bypass_doc_val, func, *args):
        ...
        if bypass_doc_val and sock_info.max_wire_version >= 4:
            raise OperationFailure(
                "Cannot set bypass_document_validation with"
    ...
    def parallel_scan(self, num_cursors, session=None, **kwargs):
        ...
        cmd = SON(
            [("parallelCollectionScan", self.__name), ("numCursors", num_cursors)]
        )
        cmd.update(kwargs)
    ...
    def ensure_index(self, key_or_list, cache_for=300, **kwargs):
        ...
        if not self.__database.client._cached(self.__database.name, self.__name, name):
            ...
            self.__database.client._cache_index(
                self.__database.name, self.__name, name, cache_for
            )
            return name
        return None
    ...
    def rename(self, new_name, session=None, **kwargs):
        ...
        if not isinstance(new_name, string_type):
            raise TypeError(
                "new_name must be an " "instance of %s" % (string_type.__name__,)
            )
    ...
    def distinct(self, key, filter=None, session=None, **kwargs):
        ...
        if not isinstance(key, string_type):
            raise TypeError(
                "key must be an " "instance of %s" % (string_type.__name__,)
            )
    ...
    def _map_reduce(self, map, reduce, out, session, read_pref, **kwargs):
        ...
        cmd = SON(
            [("mapReduce", self.__name), ("map", map), ("reduce", reduce), ("out", out)]
        )
        collation = validate_collation_or_none(kwargs.pop("collation", None))
        cmd.update(kwargs)
    ...
    def map_reduce(self, map, reduce, out, full_response=False, session=None, **kwargs):
        ...
        if not isinstance(out, (string_type, abc.Mapping)):
            raise TypeError(
                "'out' must be an instance of "
                "%s or a mapping" % (string_type.__name__,)
            )

        response = self._map_reduce(
            map, reduce, out, session, ReadPreference.PRIMARY, **kwargs
        )
    ...
    def inline_map_reduce(
        self, map, reduce, full_response=False, session=None, **kwargs
    ):
        ...
        .. versionchanged:: 3.4
           Added the `collation` option.
        """
        res = self._map_reduce(
            map, reduce, {"inline": 1}, session, self.read_preference, **kwargs
        )
        ...
        **DEPRECATED** - Use :meth:`find_one_and_delete`,
        ...
        if sort:
            ...
            # don't break existing code already using find_and_modify.
                ...
                isinstance(sort, ORDERED_TYPES)
                or isinstance(sort, dict)
                and len(sort) == 1
            ):
                warnings.warn(
