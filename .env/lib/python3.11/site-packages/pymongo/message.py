...
from bson.raw_bson import _inflate_bson, DEFAULT_RAW_BSON_OPTIONS, RawBSONDocument
...
def _insert(collection_name, docs, check_keys, flags, opts):
    ...
    return b"".join(
        [_pack_int(flags), _make_c_string(collection_name), b"".join(encoded)]
    ), max(map(len, encoded))
...
def _op_msg(
    flags, command, dbname, read_preference, secondary_ok, check_keys, opts, ctx=None
):
    """Get a OP_MSG message."""
    command["$db"] = dbname
...
def _delete(collection_name, spec, opts, flags):
    ...
    return b"".join(
        [_ZERO_32, _make_c_string(collection_name), _pack_int(flags), encoded]
    ), len(encoded)
