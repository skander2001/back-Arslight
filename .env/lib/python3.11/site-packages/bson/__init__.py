...
if PY3:
    ...
    def _encode_bytes(name, value, dummy0, dummy1):
        ...
        try:
            ...
            raise InvalidStringData(
                "strings in documents must be valid " "UTF-8: %r" % (value,)
            )
...
def _encode_mapping(name, value, check_keys, opts):
    ...
    data = b"".join(
        [_element_to_bson(key, val, check_keys, opts) for key, val in iteritems(value)]
    )
    ...
    try:
        while position < end:
            ...
            if use_raw:
                ...
                docs.append(
                    _elements_to_dict(data, view, position + 4, obj_end, codec_options)
                )
            position += obj_size
        return docs
    except InvalidBSON:
